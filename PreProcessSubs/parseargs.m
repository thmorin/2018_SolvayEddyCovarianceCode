...
%
% Similar to Matlab function setfield, but accepts multiple name-value 
% pairs and provides some additional features:
% 1) If any field of X is a cell-array of strings, it can only be set to
%    one of those strings. If no value is specified for that field, the
%    first string is selected.
% 2) Where the field is not empty, it's data type cannot be changed.
% 3) Where the field contains a scalar, it's size cannot be changed.
%
    ...
    if ~isempty(fieldind)
        ...
        if iscell(oldvalue)
            ...
            if ~iscellstr(oldvalue)
                error(sprintf('Error in parseargs function (user defined): All allowed values for "%s" must be strings',fieldname));  %#ok
            end
            if ~ischar(newvalue)
                error(sprintf('Error in parseargs function (user defined): New value for "%s" must be a string',fieldname));  %#ok
            end
            if isempty(find(strcmp(oldvalue,newvalue))) %#ok
                error(sprintf('Error in parseargs function (user defined): "%s" is not allowed for field "%s"',newvalue,fieldname));  %#ok
            end
        elseif ~isempty(oldvalue)
            ...
            if ~strcmp(class(oldvalue),class(newvalue))
                error(sprintf('Error in parseargs function (user defined): Cannot change class of field "%s" from "%s" to "%s"',...
                    fieldname,class(oldvalue),class(newvalue))); %#ok
            elseif numel(oldvalue)==1 & numel(newvalue)~=1 %#ok
                error(sprintf('Error in parseargs function (user defined): New value for "%s" must be a scalar',fieldname));  %#ok
            end
        end
        X = setfield(X,fieldname,newvalue); %#ok
        modified(fieldind) = 1;
    else
        error(['Error in parseargs function (user defined): Not a valid field name: ' fieldname]);
    end
...
if remaining~=0
    error('Error in parseargs function (user defined): Odd number of arguments supplied.  Name-value pairs required');
end
...
for i=1:length(notmodified)
    ...
    if iscell(oldvalue)
        if ~iscellstr(oldvalue)
            error(sprintf('Error in parseargs function (user defined): All allowed values for "%s" must be strings',fieldname)); %#ok
        elseif isempty(oldvalue)
            error(sprintf('Error in parseargs function (user defined): Empty cell array not allowed for field "%s"',fieldname)); %#ok
        end
