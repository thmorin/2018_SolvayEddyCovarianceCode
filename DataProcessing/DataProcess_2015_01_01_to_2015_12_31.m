...
DOY_range=Date2DOY( 01 , 01 , 2015 ):Date2DOY( 12 , 31 , 2015 );
...
for CD = 1:length(DOY_range)
    ...
    if ~isempty(slow)
        ...
        [W1min.Tm, W1min.TmQA] = DeSpike( Sdata(:,4) ,W1minWin,Dspk.TSTD,Dspk.Tmn,Dspk.Tmx, Dspk.Ttpr);   % HMP45C Temp [ Deg C ]
        fluxW.tairQA = DataQA(W1min.TmQA,NSavg);
        Sdata((Sdata(:,5)>100 & Sdata(:,5)<110),5)=100;
        [W1min.Hum, W1min.HumQA] = DeSpike(Sdata(:,5),W1minWin,Dspk.HSTD,Dspk.Hmn,Dspk.Hmx,Dspk.Htpr );    % HMP45C Relative humidity [ % ]
        ...
        [W1min.Tcnr01, W1min.Tcnr01QA] = DeSpike(Sdata(:,13),W1minWin,Dspk.TSTD,Dspk.Tmn,Dspk.Tmx, Dspk.Ttpr );  % NR01 TC Temp [ Deg C ]
        ...
        [W1min.NetRs, W1min.NetRl, W1min.Albedo, W1min.UpTot, W1min.DownTot, W1min.NetRad, W1min.IRUp, W1min.IRDown, W1min.SWUp, W1min.SWDown, QAstruct] = ...
            calculate_radiation( Sdata(:,9), Sdata(:,10), Sdata(:,11), Sdata(:,12) , W1min.Tcnr01, Dspk);
        ...
        [W1min.Total_PAR, W1min.Total_PARQA] = DeSpike( Sdata(:,14),W1minWin,Dspk.TPSTD,Dspk.TPmn,Dspk.TPmx,Dspk.TPtpr);  %BF5 Total PAR [W/m^2](1 mV = 0.5 W/m^2)
        fluxW.Total_PARQA = DataQA(W1min.Total_PARQA,NSavg);
        W1min.Total_PAR = 0.5*W1min.Total_PAR;
        [W1min.Diffuse_PAR, W1min.Diffuse_PARQA] = DeSpike( Sdata(:,15),W1minWin,Dspk.DPSTD,Dspk.DPmn,Dspk.DPmx,Dspk.DPtpr);  %BF5 Diffuse PAR [W/m^2](1 mV = .5 W/m^2)
...
%         fluxW.C3QA = DataQA (W1min.C3QA, NSavg); 
        ...
        [W1min.STat8cmOW_W1, W1min.STat8cmOW_W1QA] = DeSpike(Sdata(:,16),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);  % 107L soil temperature: 8cm into soil, in wetland 1, under open water
        fluxW.STat8cmOW_W1QA = DataQA (W1min.STat8cmOW_W1QA, NSavg);
        [W1min.STat25cmOW_W1, W1min.STat25cmOW_W1QA] = DeSpike(Sdata(:,17),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);  % 107L soil temperature: 25cm into soil, in wetland 1, under open water
        fluxW.STat25cmOW_W1QA = DataQA (W1min.STat25cmOW_W1QA, NSavg);
        [W1min.STat8cmOWr_W1, W1min.STat8cmOWr_W1QA] = DeSpike(Sdata(:,18),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);  % 107L soil temperature: 8cm into soil, in wetland 1, under open water replica
        fluxW.STat8cmOWr_W1QA = DataQA (W1min.STat8cmOWr_W1QA, NSavg);
        [W1min.STat8cmIM_W1, W1min.STat8cmIM_W1QA] = DeSpike(Sdata(:,19),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);    % 107L soil temperature: 8cm into soil, in wetland 1, intermediate
        fluxW.STat8cmIM_W1QA = DataQA (W1min.STat8cmIM_W1QA, NSavg);
        [W1min.STat25cmIM_W1, W1min.STat25cmIM_W1QA] = DeSpike(Sdata(:,20),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);   % 107L soil temperature: 25cm into soil, in wetland 1, intermediate
        fluxW.STat25cmIM_W1QA = DataQA (W1min.STat25cmIM_W1QA, NSavg);
        [W1min.STat8cmIMr_W1, W1min.STat8cmIMr_W1QA] = DeSpike(Sdata(:,21),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);   % 107L soil temperature: 8cm into soil, in wetland 1, intermediate replica
        fluxW.STat8cmIMr_W1QA = DataQA (W1min.STat8cmIMr_W1QA, NSavg);
        [W1min.STat8cmUL_W1, W1min.STat8cmUL_W1QA] = DeSpike(Sdata(:,22),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);     % 107L soil temperature: 8cm into soil, in wetland 1, upland
        fluxW.STat8cmUL_W1QA = DataQA (W1min.STat8cmUL_W1QA, NSavg);
        [W1min.STat8cmOW_W2, W1min.STat8cmOW_W2QA] = DeSpike(Sdata(:,23),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);      % 107L soil temperature: 8cm into soil, in wetland 2, under open water
        fluxW.STat8cmOW_W2QA = DataQA (W1min.STat8cmOW_W2QA, NSavg);
        [W1min.STat25cmOW_W2, W1min.STat25cmOW_W2QA] = DeSpike(Sdata(:,24),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);   % 107L soil temperature: 25cm into soil, in wetland 2, under open water
        fluxW.STat25cmOW_W2QA = DataQA (W1min.STat25cmOW_W2QA, NSavg);
        [W1min.STat8cmOWr_W2, W1min.STat8cmOWr_W2QA] = DeSpike(Sdata(:,25),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);   % 107L soil temperature: 8cm into soil, in wetland 2, under open water replica
        fluxW.STat8cmOWr_W2QA = DataQA (W1min.STat8cmOWr_W2QA, NSavg);
        [W1min.STat8cmIM_W2, W1min.STat8cmIM_W2QA] = DeSpike(Sdata(:,26),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);     % 107L soil temperature: 8cm into soil, in wetland 2, intermediate
        fluxW.STat8cmIM_W2QA = DataQA (W1min.STat8cmIM_W2QA, NSavg);
        [W1min.STat25cmIM_W2, W1min.STat25cmIM_W2QA] = DeSpike(Sdata(:,27),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);    % 107L soil temperature: 25cm into soil,in wetland 2, intermediate
        fluxW.STat25cmIM_W2QA = DataQA (W1min.STat25cmIM_W2QA, NSavg);
        [W1min.STat8cmIMr_W2, W1min.STat8cmIMr_W2QA] = DeSpike(Sdata(:,28),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);   % 107L soil temperature: 8cm into soil,in wetland 2, intermediate replica
        fluxW.STat8cmIMr_W2QA = DataQA (W1min.STat8cmIMr_W2QA, NSavg);
        [W1min.STat8cmUL_W2, W1min.STat8cmUL_W2QA] = DeSpike(Sdata(:,29),W1minWin,Dspk.STSTD,Dspk.STmn,Dspk.STmx,Dspk.STtpr);     % 107L soil temperature: 8cm into soil,in wetland 2, upland
        fluxW.STat8cmUL_W2QA = DataQA (W1min.STat8cmUL_W2QA, NSavg);
        
        [W1min.WD_min, W1min.WD_min_QA] = DeSpike(Sdata(:,30),W1minWin,6,0,360,1);
        fluxW.WD_min_QA = DataQA (W1min.WD_min_QA, NSavg);
        [W1min.WD_bar, W1min.WD_bar_QA] = DeSpike(Sdata(:,31),W1minWin,6,0,360,1);
        fluxW.WD_bar_QA = DataQA (W1min.WD_bar_QA, NSavg);
        [W1min.WD_max, W1min.WD_max_QA] = DeSpike(Sdata(:,35),W1minWin,6,0,360,1);
        fluxW.WD_max_QA = DataQA (W1min.WD_max_QA, NSavg);
        
        [W1min.WS_min, W1min.WS_min_QA] = DeSpike(Sdata(:,34),W1minWin,Dspk.VWSTD,Dspk.VPmn,Dspk.VWmx,Dspk.VWtpr);
        fluxW.WS_min_QA = DataQA (W1min.WS_min_QA, NSavg);
        
        [W1min.WS_bar, W1min.WS_bar_QA] = DeSpike(Sdata(:,33),W1minWin,Dspk.VWSTD,Dspk.VPmn,Dspk.VWmx,Dspk.VWtpr);
        fluxW.WS_bar_QA = DataQA (W1min.WS_bar_QA, NSavg);
        
        [W1min.WS_max, W1min.WS_max_QA] = DeSpike(Sdata(:,35),W1minWin,Dspk.VWSTD,Dspk.VPmn,Dspk.VWmx,Dspk.VWtpr);
        fluxW.WS_max_QA = DataQA (W1min.WS_max_QA, NSavg);
        
        [W1min.tair_2, W1min.tair_2_QA] = DeSpike(Sdata(:,38),W1minWin,Dspk.TSTD,Dspk.Tmn,Dspk.Tmx,Dspk.Ttpr);
        fluxW.tair_2_QA = DataQA (W1min.tair_2_QA, NSavg);
        
        [W1min.rH_2, W1min.rH_2_QA] = DeSpike(Sdata(:,39),W1minWin,6,0,100,1);
        fluxW.rH_2_QA = DataQA (W1min.rH_2_QA, NSavg);
        
        [W1min.pres,W1min.pres_QA] = DeSpike(Sdata(:,40),W1minWin,6,900,1200,1);
        fluxW.pres_QA = DataQA (W1min.pres_QA, NSavg);
        
        W1min.rain_accum    = Sdata(:,41);
        W1min.rain_dur      = Sdata(:,42);
        W1min.rain_intens   = Sdata(:,43);
        W1min.hail_accum    = Sdata(:,44);
        W1min.hail_dur      = Sdata(:,45);
        W1min.hail_intens   = Sdata(:,46);
        
        [W1min.C1, W1min.C1_QA] = DeSpike(Sdata(:,49),W1minWin,Dspk.CO2STD,Dspk.CO2mn,Dspk.CO2mx,Dspk.CO2tpr);
        fluxW.C1_QA = DataQA (W1min.C1_QA, NSavg);
        [W1min.C2, W1min.C2_QA] = DeSpike(Sdata(:,50),W1minWin,Dspk.CO2STD,Dspk.CO2mn,Dspk.CO2mx,Dspk.CO2tpr);
...
%     [W1min.DS2_gust, W1min.DS2_gustQA] = DeSpike (DsData(:,35),W1minWin,Dspk.VWSTD,Dspk.VWmn,50,Dspk.VWtpr);                % DS2 sonic anemometer gust speed [m/s]
    ...
    if ~isempty(fast)
        ...
        [DespikeW.LI7500_C2, DespikeW.LI7500_CQA2] = DeSpike (Fdata(:,14)/44,3600,Dspk.CSTD,Dspk.Cmn,Dspk.Cmx,Dspk.Ctpr,'EC150',Fdata(:,16),Fdata(:,19),RSSIper);      % LI7500 CO2 [mmol/m^3] { Fdata(:,17) = LI7500 Diag}
        ...
        [DespikeW.LI7500_Q2, DespikeW.LI7500_QQA2] = DeSpike (Fdata(:,15)*1000/18,3600,Dspk.QSTD,Dspk.Qmn,Dspk.Qmx,Dspk.Qtpr,'EC150',Fdata(:,16),Fdata(:,20),RSSIper);    % LI7500 H2O [mmol/m^3]
        ...
        [DespikeW.LI7500_P2, DespikeW.LI7500_PQA2] = DeSpike(Fdata(:,18),2400,Dspk.PSTD,Dspk.Pmn,Dspk.Pmx,Dspk.Ptpr,'EC150',Fdata(:,16));     % LI7500 Pressure [ Pa ]
...
[DespikeW.CSAT_U, DespikeW.CSAT_UQA]= DeSpike(Fdata(:, 21),2400,Dspk.VWSTD,Dspk.VWmn,Dspk.VWmx,Dspk.VWtpr,'CSAT3',Fdata(:,25)); % CSAT Ux [m/s]
        ...
        [DespikeW.LI7500_P, DespikeW.LI7500_PQA] = DeSpike(Fdata(:,28),2400,Dspk.PSTD,Dspk.Pmn,Dspk.Pmx,Dspk.Ptpr,'LI7500',Fdata(:,29));     % LI7500 Pressure [ Pa ]
...
%         if CD==1 || CD==2
%             DespikeW.LI7700_M(1:length(DespikeW.LI7700_M)/2)=((DespikeW.LI7700_M(1:length(DespikeW.LI7700_M)/2)/S_old+Z_old)-Z_new)*S_new;
%         end
