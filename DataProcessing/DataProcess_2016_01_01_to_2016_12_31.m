...
DOY_range=Date2DOY( 04 , 01 , 2016 ):Date2DOY( 12 , 31 , 2016 );
...
%     [W1min.DS2_gust, W1min.DS2_gustQA] = DeSpike (DsData(:,35),W1minWin,Dspk.VWSTD,Dspk.VWmn,50,Dspk.VWtpr);                % DS2 sonic anemometer gust speed [m/s]
    ...
    if ~isempty(fast)
        ...
        [DespikeW.LI7500_C, DespikeW.LI7500_CQA] = DeSpike (Fdata(:,14)/44,3600,Dspk.CSTD,Dspk.Cmn,Dspk.Cmx,Dspk.Ctpr,'EC150',Fdata(:,16),Fdata(:,19),RSSIper);      % LI7500 CO2 [mmol/m^3] { Fdata(:,17) = LI7500 Diag}
        ...
        [DespikeW.LI7500_Q, DespikeW.LI7500_QQA] = DeSpike (Fdata(:,15)*1000/18,3600,Dspk.QSTD,Dspk.Qmn,Dspk.Qmx,Dspk.Qtpr,'EC150',Fdata(:,16),Fdata(:,20),RSSIper);    % LI7500 H2O [mmol/m^3]
        ...
        [DespikeW.LI7500_P, DespikeW.LI7500_PQA] = DeSpike(Fdata(:,18),2400,Dspk.PSTD,Dspk.Pmn,Dspk.Pmx,Dspk.Ptpr,'EC150',Fdata(:,16));     % LI7500 Pressure [ Pa ]
        fluxW.LI7500_PQA = DataQA (DespikeW.LI7500_PQA, NFavg);
        DespikeW.LI7500_P=1000*DespikeW.LI7500_P;

Fdata(Fdata(:,13)>70,25) = nan;
[DespikeW.CSAT_U, DespikeW.CSAT_UQA]= DeSpike(Fdata(:, 9),2400,Dspk.VWSTD,Dspk.VWmn,Dspk.VWmx,Dspk.VWtpr,'CSAT3',Fdata(:,13)); % CSAT Ux [m/s]
        fluxW.CSAT_UQA = DataQA (DespikeW.CSAT_UQA, NFavg);
        [DespikeW.CSAT_V, DespikeW.CSAT_VQA] = DeSpike(Fdata(:,10),2400,Dspk.VWSTD,Dspk.VWmn,Dspk.VWmx,Dspk.VWtpr,'CSAT3',Fdata(:,13)); % CSAT Uy [m/s]
        fluxW.CSAT_VQA = DataQA (DespikeW.CSAT_VQA, NFavg);
        [DespikeW.CSAT_W, DespikeW.CSAT_WQA] = DeSpike(Fdata(:,11),2400,Dspk.HWSTD,Dspk.HWmn,Dspk.HWmx,Dspk.HWtpr,'CSAT3',Fdata(:,13)); % CSAT Uz [m/s]
        fluxW.CSAT_WQA = DataQA (DespikeW.CSAT_WQA, NFavg);
        [DespikeW.CSAT_Tmp, DespikeW.CSAT_TmpQA] = DeSpike(Fdata(:,12),3600,Dspk.TSTD,Dspk.Tmn,Dspk.Tmx, Dspk.Ttpr,'CSAT3',Fdata(:,13)); % CSAT Ts [Deg C]
