...
function [W1min,DespikeW,tmpTEMPRMY,fluxW,fluxWind,theta,alfa,u_RMY,v_RMY,w_RMY,u_CSAT,v_CSAT,w_CSAT,qWPL,cWPL,mWPL,Sdata,Fdata,Header,Tr_CSAT,Tr_RMY]...
    ...
    DespikeW.RMY_U = nan(ntsf,1);        % RMYoung Wind in U direction
    DespikeW.RMY_UQA = ones(ntsf,1)*6;
    DespikeW.RMY_V = nan(ntsf,1);        % RMYoung Wind in V direction
    DespikeW.RMY_VQA = ones(ntsf,1)*6;
    DespikeW.RMY_W = nan(ntsf,1);        % RMYoung Wind in W direction
    DespikeW.RMY_WQA = ones(ntsf,1)*6;
    tmpTEMPRMY = nan(ntsf,1);
    DespikeW.RMY_Tmp = nan(ntsf,1);      % RMYoung Sonic Temperature
    DespikeW.RMY_TmpQA = ones(ntsf,1)*6;
