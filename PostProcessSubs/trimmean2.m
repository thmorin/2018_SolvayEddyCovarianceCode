function [m, stand] = trimmean2(x,percent,flag,dim)
%TRIMMEAN2 Trimmed mean and trimmed standard deviation
...
if nargin < 2
    error(message('stats:trimmean2:TooFewInputs'));
elseif ~isscalar(percent) || percent >= 100 || percent < 0
    error(message('stats:trimmean2:InvalidPercent'));
end
...
if ~any(isnan(x(:)))
    ...
    n = size(x,1);
    [m,alltrimmed,stand] = F(x,n,percent,sz);
else
    ...
    for j = 1:prod(sz(2:end))
        ...
        [m(j),alltrimmed(j),stand] = F(x(:,j),n,percent,[1 1]);
    end
end
...
function [m,alltrimmed,stand] = roundtrim(x,n,percent,sz)
k = n*percent/200;
k0 = round(k - eps(k));
if ~isempty(n) && n>0 && k0<n/2
    m = mean(x((k0+1):(n-k0),:),1);
    alltrimmed = false;
    stand = std(x((k0+1):(n-k0),:));
else
    m = NaN(sz,class(x));
    alltrimmed = true;
    stand=nan;
...
function [m,alltrimmed,stand] = unwtdtrim(x,n,percent,sz)
...
if ~isempty(n) && n>0 && k0<n/2
    ...
    m = NaN(sz,class(x));
    alltrimmed = true;
    stand = NaN(sz,class(x));
end

% --- Weight observations to achieve desired percentage trimming
function [m,alltrimmed,stand] = wtdtrim(x,n,percent,sz)
